...
### 1. var는 왜 '동네방네' 다 알게 될까요? (전역 변수)
    ...
    설명 : var는 아주 옛날 방식의 이름표예요. 집 안(함수 안)에서 만들지 않고 밖에서 만들면,
    프로그램 어디에서든 이 이름을 쓸 수 있게 돼요.
    이걸 '전역 변수'라고 해요.
...
// 함수를 정의할 때는 function 키워드를 사용하고,
// 함수 이름과 매개변수(필요한 경우)를 지정합니다.
// 함수 내부에서는 return 키워드를 사용하여 결과 값을 반환할 수 있습니다.
// 함수를 호출할 때는 함수 이름과 괄호를 사용하며,
// 필요한 인자를 괄호 안에 전달합니다.
// 함수는 코드의 재사용성을 높이고, 프로그램의 구조를 개선하는 데 도움이 됩니다.
